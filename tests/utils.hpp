...
template <template <class, class> class Cntr, class T, template <typename> class Alloc>
int	timeTest(const char* testName, int (*f)(unsigned int, Cntr<T, Alloc<T> >&), Cntr<T, Alloc<T> >& cntr, unsigned int seed) {
  std::clock_t start_time = std::clock();
  int result = 0;
  try {
    result = (*f)(seed, cntr);
